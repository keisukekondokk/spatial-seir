...
foreach(i = 0:47, .packages = c("scales", "ggplot2", "dplyr", "ggrepel")) %dopar% {
  ...
  dfTemp1 <- dfResultsLong1 %>%
    ...
    dplyr::mutate(Beta = if_else(date < startDay, NA_real_, Beta))
  #Dataframe of List 2
  ...
  #ggplot2
  ggplot() +
    geom_line(aes(x = date, y = Beta),
              size = 1.3, 
              linetype = "solid",
              data = dfTemp2) +
    geom_point(aes(x = date-1, y = Beta), data = dfTemp3, size = 2, shape = 22, fill="transparent", stroke = 2, color = "red") +
    geom_label_repel(aes(x = date-1, y = Beta, label = as.character(date-1)), data = dfTemp3, vjust = -3, hjust = 1, size = 3) +
    scale_color_manual(values = colorLine) + 
    scale_y_continuous(breaks=seq(0, 0.2, by = 0.05), limits=c(0, 0.2), labels = comma_format(accuracy = .01)) +
    scale_x_date(date_breaks = "3 months",
                 date_labels = "%Y\n%m-%d") + 
    ylab("Time-Varying Transmission Rate") +
    xlab("") +
    theme_bw() +

...
foreach(i = 0:47, .packages = c("scales", "ggplot2", "dplyr", "ggrepel", "stringr")) %dopar% {
  ...
  ggplot() +
    ...
    geom_line(aes(x = date, y = Ratio, color = labelLine1), 
              ...
              linetype = "solid",
              data = dfTemp1) +
    geom_point(aes(x = date, y = Ratio), data = dfTemp3, size = 2, shape = 22, fill="transparent", stroke = 2, color = "red") +
    geom_label_repel(aes(x = date, y = Ratio, label = labelDate), data = dfTemp3, vjust = -3, hjust = 1, size = 6) +

...
foreach(i = 0:numPref, .packages = c("scales", "ggplot2", "dplyr", "ggrepel")) %dopar% {
  ...
  dfTemp1 <- dfResultsLong1 %>%
    ...
    dplyr::filter(date >= startDayFigure) %>%
    dplyr::filter(date <= endDayFigure) %>%
    dplyr::mutate(E = if_else(date < startDay, NA_real_, E))
  #Dataframe of List 2
  ...
  #Label Location
  nudge_y_ggrepel <- 0.8 * max(dfTemp0$E, na.rm = TRUE)
  
  #ggplot2
  if(numCaseScenario == 1 | numCaseScenario == 2 ){
    ggtemp <- ggplot() +
      geom_line(aes(x = date, y = E, color = labelLine0),
                size = 1.1, 
                linetype = "solid",
                data = dfTemp0) +
      geom_line(aes(x = date, y = E, color = labelLine2),
                size = 1.3, 
                linetype = "solid",
                data = dfTemp2) +
      geom_line(aes(x = date, y = E, color = labelLine1), 
                size = 1.2, 
                linetype = "solid",
                data = dfTemp1)
  } else{
    ggtemp <- ggplot() +
      geom_line(aes(x = date, y = E, color = labelLine0),
                ...
                linetype = "solid",
                data = dfTemp0) +
      geom_line(aes(x = date, y = E, color = labelLine3),
                size = 1.3, 
                linetype = "solid",
                data = dfTemp3) +
      geom_line(aes(x = date, y = E, color = labelLine2),
  ...
  listPlot[[i+1]] <- ggtemp +
    geom_point(aes(x = date, y = E), data = dfTemp9, size = 2, shape = 22, fill="transparent", stroke = 2, color = "red") +
    geom_label_repel(aes(x = date, y = E, label = as.character(date)), data = dfTemp9, nudge_x = -50, nudge_y = nudge_y_ggrepel, size = 3) +

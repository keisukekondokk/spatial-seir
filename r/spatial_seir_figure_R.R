...
foreach(i = 0:numPref, .packages = c("scales", "ggplot2", "dplyr", "ggrepel")) %dopar% {
  ...
  if(numCaseScenario == 3){
    ...
    colorLine <- c("#2f7ed8", "#f45b5b", "#483d8b")
  }
  ...
  ggplot() +
    ...
    geom_line(aes(x = date, y = R, color = labelLine1), 
              ...
              linetype = "solid",
              data = dfTemp1) +
    geom_point(aes(x = date, y = R), data = dfTemp3, size = 2, shape = 22, fill="transparent", stroke = 2, color = "red") +
    geom_label_repel(aes(x = date, y = R, label = as.character(date)), data = dfTemp3, nudge_x = -50, nudge_y = nudge_y_ggrepel, size = 3) +

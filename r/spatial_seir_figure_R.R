...
foreach(i = 0:numPref, .packages = c("scales", "ggplot2", "dplyr", "ggrepel")) %dopar% {
  ...
  dfTemp1 <- dfResultsLong1 %>%
    dplyr::filter(prefCode == i) %>%
    dplyr::filter(date <= endDayFigure) %>%
    dplyr::mutate(R = if_else(date < startDay, NA_real_, R))
  #Dataframe of List 2
  ...
  #Label Location
  nudge_y_ggrepel <- 0.5 * max(dfTemp1$R, na.rm = TRUE)
  
  #ggplot2
  if(numCaseScenario == 1 | numCaseScenario == 2 ){
    ggtemp <- ggplot() +
      geom_line(aes(x = date, y = R, color = labelLine0),
                size = 1.1, 
                linetype = "solid",
                data = dfTemp0) +
      geom_line(aes(x = date, y = R, color = labelLine2),
                size = 1.3, 
                linetype = "solid",
                data = dfTemp2) +
      geom_line(aes(x = date, y = R, color = labelLine1), 
                size = 1.2, 
                linetype = "solid",
                data = dfTemp1)
  } else{
    ggtemp <- ggplot() +
      geom_line(aes(x = date, y = R, color = labelLine0),
                ...
                linetype = "solid",
                data = dfTemp0) +
      geom_line(aes(x = date, y = R, color = labelLine3),
                size = 1.3, 
                linetype = "solid",
                data = dfTemp3) +
      geom_line(aes(x = date, y = R, color = labelLine2),
  ...
  listPlot[[i+1]] <- ggtemp +
    geom_point(aes(x = date, y = R), data = dfTemp9, size = 2, shape = 22, fill="transparent", stroke = 2, color = "red") +
    geom_label_repel(aes(x = date, y = R, label = as.character(date)), data = dfTemp9, nudge_x = -50, nudge_y = nudge_y_ggrepel, size = 3) +
